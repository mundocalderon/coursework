class Api::ResultsController < ApplicationController 
  ...
  def index
		if !request.accept || request.accept == "*/*"
	    	...
	    	render :index, :object => @entrants, status: :ok
			end
		end  
  end
  ...
  def show
		if !request.accept || request.accept == "*/*"
			...
			render :partial=>"api/results/result", :object=>@result
		end    	
  end
  ...
  def update
	  ...
	  entrant.save
	  render plain: "/api/races/#{params[:race_id]}/results/#{params[:id]} update!!",  status: :ok
	end
